...
namespace ScreenSaver
{
	public partial class SettingsForm : Form
    {
        ...
        // Live preview of the current-time clock, rebuilt whenever a setting changes and ticked by a timer.
        private CurrentTimeScreen _previewScreen;
        ...
        public SettingsForm(FlipItSettings settings)
		{
            ...
            versionLabel.Text = $"版本 {GetVersion()}";
        }
        ...
        private void SaveSettings()
        {
            ...
            _settings.HoursScale = (int)hoursScaleNumericUpDown.Value;
            _settings.MinutesScale = (int)minutesScaleNumericUpDown.Value;
		...
		{
            // Save but keep the window open so the user can keep tweaking and watching the live preview.
            SaveSettings();
		}
        ...
        private void SettingsForm_Load(object sender, EventArgs e)
        {
            ...
            showSecondsCheckBox.Checked = _settings.ShowSeconds;

            hoursScaleNumericUpDown.Value = ClampToRange(hoursScaleNumericUpDown, _settings.HoursScale);
            minutesScaleNumericUpDown.Value = ClampToRange(minutesScaleNumericUpDown, _settings.MinutesScale);
            secondsScaleNumericUpDown.Value = ClampToRange(secondsScaleNumericUpDown, _settings.SecondsScale);
            UpdateSecondsScaleEnabled();
        ...
        {
            // Rebuild the preview whenever any current-time setting changes.
            display24hrRadioButton.CheckedChanged += PreviewSettingChanged;
            display12hrRadioButton.CheckedChanged += PreviewSettingChanged;
            scaleTrackBar.ValueChanged += PreviewSettingChanged;
            showSecondsCheckBox.CheckedChanged += PreviewSettingChanged;
            hoursScaleNumericUpDown.ValueChanged += PreviewSettingChanged;
            minutesScaleNumericUpDown.ValueChanged += PreviewSettingChanged;
            ...
            _previewTimer = new Timer { Interval = 33 }; // ~30fps is plenty for the small preview
            _previewTimer.Tick += (s, e) =>
            {
                try { _previewScreen?.Draw(); }
                catch { /* ignore transient drawing errors while resizing/closing */ }
            };
            _previewTimer.Start();
        ...
        private void RefreshPreview()
        {
            ...
            {
                try { _previewScreen.Draw(); }
                catch { /* ignore transient drawing errors (e.g. mid-resize) */ }
            }
        }
        ...
        private void worldTimesListView_AfterLabelEdit(object sender, LabelEditEventArgs e)
        {
            ...
            {
                MessageBox.Show("名称不能为空。", "FlipIt", MessageBoxButtons.OK, MessageBoxIcon.Information);
                e.CancelEdit = true;
                return;
            }
            if (newCityName.Contains("="))
            {
                MessageBox.Show("名称不能包含等号 (=)。", "FlipIt", MessageBoxButtons.OK, MessageBoxIcon.Information);
                e.CancelEdit = true;
                return;
            }
            ...
            {
                MessageBox.Show("无法更新该名称。", "FlipIt", MessageBoxButtons.OK, MessageBoxIcon.Information);
                e.CancelEdit = true;
                return;
            }
        }
        ...
        private void AddCityFromSearchBox()
        {
            ...
            if (location != null)
            {
                ...
                {
                    MessageBox.Show($"“{location.DisplayName}” 已在列表中", "重复的城市", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        {
            _settings.ShowSeconds = showSecondsCheckBox.Checked;
            UpdateSecondsScaleEnabled();
        }

        private void UpdateSecondsScaleEnabled()
        {
            // The seconds box size only matters when seconds are shown.
            secondsScaleLabel.Enabled = showSecondsCheckBox.Checked;
            secondsScaleNumericUpDown.Enabled = showSecondsCheckBox.Checked;
        }
